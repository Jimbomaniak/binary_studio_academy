...
class ImporovedFighter extends Fighter{
    doubleHit(enemy, point = 1){
        super.hit(enemy, point * 2);
    }

    toString(){
        return `Imporoved fighter ${this._name} (${this._health})`
    }
}
...
function fight(fighter, iFighter, ...point){
    ...
    var randomPoint = () => point[Math.floor(Math.random() * point.length)];
    var doubleDamage = () => Math.round(Math.random());
...
let subZero = new ImporovedFighter('Sub-Zero', 100, 1)
